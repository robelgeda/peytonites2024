...
__global__ void calcForce(double* x_arr, double* y_arr, double* z_arr, double* ax_arr, \
                          double* ay_arr, double* az_arr, double* mass_arr, unsigned int N, double soft, double G)
{
    unsigned int i = (blockDim.x * blockIdx.x) + threadIdx.x;
    double ax, ay, az; double dx, dy, dz; double mj, a;
    double xi, yi, zi; double r_ij_squared, r_ij, r_ij_cubed;
    if(i<N){
...
__global__ void updatePosition(double* vx_arr, double* vy_arr, double* vz_arr, \
                               double* x_arr, double* y_arr, double* z_arr, double* ax_arr, \
                               double* ay_arr, double* az_arr, unsigned int N, double dt)
{
    unsigned int i = (blockDim.x * blockIdx.x) + threadIdx.x;
    if(i<N){
...
calcForceKernel = cp.RawKernel(r'''
extern "C" __global__
void calcForce(double* x_arr, double* y_arr, double* z_arr, double* ax_arr, \
                          double* ay_arr, double* az_arr, double* mass_arr, unsigned int N, double soft, double G)
{
    extern __shared__ double tmp[];

    int bId = blockIdx.x, bDim = blockDim.x;
    int tId = threadIdx.x;
    int val = (N+bDim-1)/bDim;

    double ax, ay, az; double dx, dy, dz; double mj, a;
    double xi, yi, zi; double r_ij_squared, r_ij, r_ij_cubed;

    tmp[tId+(0*bDim)] = 0.0; tmp[tId+(1*bDim)] = 0.0; tmp[tId+(2*bDim)] = 0.0;
    xi = x_arr[bId]; yi = y_arr[bId]; zi = z_arr[bId];

    for(int itr=0;itr<val;itr++){
        int j = tId+(itr*bDim);
        if(j<N){
            if(bId!=j){
                dx = xi - x_arr[j];
...
updatePositionKernel = cp.RawKernel(r'''
extern "C" __global__ 
void updatePosition(double* vx_arr, double* vy_arr, double* vz_arr, \
                               double* x_arr, double* y_arr, double* z_arr, double* ax_arr, \
                               double* ay_arr, double* az_arr, unsigned int N, double dt)
{
    unsigned int i = (blockDim.x * blockIdx.x) + threadIdx.x;
...
def simulation(sim_init_cond, out_dir, verbose=True):
    ...
    #cuPy Arrays
    cu_x_arr = cp.array(x_arr, copy=True)
    cu_y_arr = cp.array(y_arr, copy=True)
    cu_z_arr = cp.array(z_arr, copy=True)

    cu_vx_arr = cp.array(vx_arr, copy=True)
    cu_vy_arr = cp.array(vy_arr, copy=True)
    cu_vz_arr = cp.array(vz_arr, copy=True)

    cu_mass_arr = cp.array(mass_arr, copy=True)
...
#Computation Start
    ...
    for step in range(nsteps):
        ...
        xThreads = 128
        grid = (number_particles,)
        block = (xThreads,)
        calcForceKernel(grid,block,(cu_x_arr,cu_y_arr,cu_z_arr,cu_ax_arr,cu_ay_arr,cu_az_arr,cu_mass_arr,number_particles,soft,G),shared_mem=xThreads*8*3)
