...
extern "C" __global__
void calcForce(float* x_arr, float* y_arr, float* z_arr, float* ax_arr, \
                          float* ay_arr, float* az_arr, float* mass_arr, unsigned int N, float soft, float G)
{
...
extern "C" __global__ 
void updatePosition(float* vx_arr, float* vy_arr, float* vz_arr, \
                               float* x_arr, float* y_arr, float* z_arr, float* ax_arr, \
                               float* ay_arr, float* az_arr, unsigned int N, float dt)
{
    unsigned int i = (blockDim.x * blockIdx.x) + threadIdx.x;
...
#Computation Start
    ...
    for step in range(nsteps):
        ...
        block = (xThreads,)
        calcForceKernel(grid,block,(cu_x_arr,cu_y_arr,cu_z_arr,cu_ax_arr,cu_ay_arr,cu_az_arr,cu_mass_arr,number_particles,soft,G),shared_mem=xThreads*4*3)
